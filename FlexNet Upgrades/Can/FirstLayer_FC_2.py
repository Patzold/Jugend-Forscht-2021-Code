...
pickle_in = open(save_dir + "can_intm_2_raw.pickle","rb")
train = pickle.load(pickle_in)
pickle_in = open(save_dir + "can_intm_2t_raw.pickle","rb")
...
def evaluate():
    ...
    with torch.no_grad():
        for i in tqdm(range(len(Xt))):
            ...
            net_out = net(Xt[i].view(-1, 4).to(device))[0]  # returns a list
            predicted_class = torch.argmax(net_out)
            if predicted_class == real_class:
                correct += 1
...
for epoch in range(EPOCHS):
    ...
    if osample > valid_acc_min and epoch > 10:
        ...
        torch.save(net.state_dict(), "C:/Cache/PJF-30/can_intm_2.pt") #                                                  <-- UPDATE
...
# Max Out of Sample Accuracy: 0.719    5min 16s          4 - 96 - 32 - 4
 # Max Out of Sample Accuracy: 0.857    2min 57s         3 - 48 - 16 - 3
# Max Out of Sample Accuracy: 0.719    5min 27s         4 - 24 - 8 - 4
