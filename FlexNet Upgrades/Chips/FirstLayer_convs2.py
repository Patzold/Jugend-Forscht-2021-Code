...
rubt, pig, lego, chips = RubberToy(), PigHead(), Lego(), Chips()
...
chips.load_state_dict(torch.load("C:/Cache/PJF-30/categorys_chips_1_1.pt"))
rubt.to(device)
pig.to(device)
lego.to(device)
chips.to(device)
...
def run(input_tensor):
    with torch.no_grad():
        ...
        chips_out = chips(input_tensor).cpu().numpy().tolist()[0]
        chips_argmax = torch.argmax(chips(input_tensor).cpu()).numpy().tolist()
        out = [rubt_argmax, pig_argmax, lego_argmax, chips_argmax] + rubt_out + pig_out + lego_out + chips_out # v3, all
        return out