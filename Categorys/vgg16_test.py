...
print("In features: ", vgg16.classifier[0].in_features)
print("Out features: ", vgg16.classifier[6].out_features)

# Freeze training for all "features" layers
for indx, param in enumerate(vgg16.features.parameters()):
    if indx < 12:
...
first_layer = nn.Linear(n_inputs, 800)
drop1 = nn.Dropout(0.75)
middle_layer = nn.Linear(800, 100)
drop2 = nn.Dropout(0.75)
last_layer = nn.Linear(100, 2)

vgg16.classifier[0] = first_layer
vgg16.classifier[2] = drop1
vgg16.classifier[3] = middle_layer
vgg16.classifier[5] = drop2
...
def evaluate():
    ...
    with torch.no_grad():
        for i in tqdm(range(len(Xt))):
            ...
            net_out = vgg16(Xt[i].view(-1, 3, 224, 224).to(device))[0]  # returns a list
            predicted_class = torch.argmax(net_out)
            # print(real_class, net_out, predicted_class)
            # input()
            if predicted_class == real_class:
