...
def evaluate():
    ...
    with torch.no_grad():
        for i in tqdm(range(len(eval_X))):
            ...
            net_out = net(eval_X[i].view(-1, 2).to(device))[0]  # returns a list
    ...
    with torch.no_grad():
        for i in tqdm(range(len(Xt))):
            ...
            net_out = net(Xt[i].view(-1, 2).to(device))[0]  # returns a list
            predicted_class = torch.argmax(net_out)
            if predicted_class == real_class:
                correct += 1
...
for epoch in range(EPOCHS):
    ...
    if osample > valid_acc_min and epoch > 10:
        ...
        torch.save(net.state_dict(), "C:/Cache/PJF-30/intm_2.pt") #                                                  <-- UPDATE
...
# Max Out of Sample Accuracy: 0.803    4min 19s         2 - 192 - 96 - 3
# Max Out of Sample Accuracy: 0.803    4min 18s         2 - 96 - 48 - 3
# Max Out of Sample Accuracy: 0.726    3min 53s         2 - 48 - 24 - 3
# Max Out of Sample Accuracy: 0.794    4min 16s         2 - 24 - 8 - 3
